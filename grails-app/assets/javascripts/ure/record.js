...
// repair some escapes
!function() {
  var content_container = ".reccellcontent";
...
!function() {
  ...
  no_token_fields = [ 'munsell_color', 'date_edited' ];
  data_attr = 'data-ure-record-field'
  stops = [ "br", "a", "a's", "able", "about", "above", "according", "accordingly", "across", "actually", "after",
  ...
  $(document).ready(function() {
    $(container).each(function() {
      ...
      var field = $(this).attr(data_attr);
      if ($.inArray(field, exclude_fields) < 0 && $.inArray(field, no_token_fields) < 0) {

        var url = "/recordlist/" + field + "/";
        var reccell = $(this).find(content_container)
        var html = reccell.html()
      ...
      if ($.inArray(field, no_token_fields) > -1) {
        ...
        if (html !== null) {
          ...
          var html = '<a href="' + url + html.replace('/', '--').trim() + '">' + html.trim() + '</a>';
          $(reccell).html(html);
        }
      }
...
// matcher -- values are written as js globals in header of this template
...
!function() {
  ...
  $(document).ready(
      function() {
        ...
        if (uredb_matcher_hasMatch) {
          if (uredb_matcher_matchfield === 'munsell_color') {
